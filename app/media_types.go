...
// Histogram bucket (default view)
//
// Identifier: application/vnd.aerospike.amc.namespace.histbucket.response+json; view=default
type AerospikeAmcNamespaceHistbucketResponse struct {
	// count of the records whose values falls in this bucket [min, max]
	Count int `form:"count" json:"count" xml:"count"`
	// values greater than max are not counted towards this bucket
	Max int `form:"max" json:"max" xml:"max"`
	// values less than min are not counted towards this bucket
	Min int `form:"min" json:"min" xml:"min"`
}

// Validate validates the AerospikeAmcNamespaceHistbucketResponse media type instance.
func (mt *AerospikeAmcNamespaceHistbucketResponse) Validate() (err error) {
...
type AerospikeAmcNotificationResponse struct {
	// Notifications
	Notifications []*Notification `form:"notifications" json:"notifications" xml:"notifications"`
	// UnresolvedNotification Count
	UnresolvedCount int `form:"unresolvedCount" json:"unresolvedCount" xml:"unresolvedCount"`
}
...
func (mt *AerospikeAmcNotificationResponse) Validate() (err error) {
	...
	if mt.Notifications == nil {
		err = goa.MergeErrors(err, goa.MissingAttributeError(`response`, "notifications"))
	}
	for _, e := range mt.Notifications {
		if e != nil {
			if err2 := e.Validate(); err2 != nil {
				err = goa.MergeErrors(err, err2)
			}
