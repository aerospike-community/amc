...
func MountConnectionController(service *goa.Service, ctrl ConnectionController) {
	...
	h = func(ctx context.Context, rw http.ResponseWriter, req *http.Request) error {
		...
		// Build the context
		rctx, err := NewGetLogsConnectionContext(ctx, req, service)
		if err != nil {
			return err
		}
...
func MountNodeController(service *goa.Service, ctrl NodeController) {
	...
	h = func(ctx context.Context, rw http.ResponseWriter, req *http.Request) error {
		...
		// Build the context
		rctx, err := NewGetLogsNodeContext(ctx, req, service)
		if err != nil {
			return err
		}
		return ctrl.GetLogs(rctx)
...
func MountPublicController(service *goa.Service, ctrl PublicController, path string) {
	initService(service)
	var h goa.Handler
	...
	h = ctrl.FileHandler("/*filepath", path)
	h = handlePublicOrigin(h)
	service.Mux.Handle("GET", "/*filepath", ctrl.MuxHandler("serve", h, nil))
	service.LogInfo("mount", "ctrl", "Public", "files", path, "route", "GET /*filepath")

	h = ctrl.FileHandler("/", index)
	h = handlePublicOrigin(h)
	service.Mux.Handle("GET", "/", ctrl.MuxHandler("serve", h, nil))
	service.LogInfo("mount", "ctrl", "Public", "files", index, "route", "GET /")
}
