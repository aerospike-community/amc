...
define(["jquery", "underscore", "backbone" , "poller", "models/dashboard/clusterwidenamespacemodel", "views/dashboard/clusterwidenamespaceview", 'helper/namespace-table', "helper/namespace-clusterwide-table","config/app-config", "config/view-config", "helper/util", "helper/jqgrid-helper"], function($, _, Backbone, Poller, NamespaceModel, NamespaceView, NamespaceTable, NamespaceClusterWideTable, AppConfig, ViewConfig, Util, GridHelper){
    var NamespaceCollection = Backbone.Collection.extend({
        ...
        initialize : function(options){
            try{
                ...
                NamespaceClusterWideTable.initClusterWideNamespaceGrid(AppConfig.namespace.namespaceClusterWideTableDiv, this);
        ...
        initVariables :function(options){
            ...
            this.namespaces = options["namespace_list"];
            this.rowID = 0;
        ...
        url: function(){
            return AppConfig.baseUrl +
                window.AMCGLOBALS.persistent.clusterID +
                AppConfig.namespace.resourceUrl + this.namespaces.toString() +
                ((!_.isEmpty(window.AMCGLOBALS.persistent.selectedNodes)) ? ("?nodes=" + window.AMCGLOBALS.persistent.selectedNodes.toString()) : "");
        },

        onModelAdded: function(model, collection, options){
            collection.viewSortRequired = true;
            model.set("row_id", collection.rowID++);
            model.rowView = new NamespaceView({model: model, el : "#" + AppConfig.namespace.namespaceClusterWideTablePrefix + model.get("row_id")});
        },
        ...
        parse: function(resp, options) {
            //function parses 'resp' to copy namespace name key as an name attribute of the same object
            //This name works as ID for the models thus helps in mapping to pre-existing models
            //Each of these namespace objects are then mapped to an array, making Backbone.collection identify them as different models

            var models = [];
            for(var namespace in resp){
                resp[namespace].name = namespace;
                models.push(resp[namespace]);
            }
            return models;
        },
        ...
        comparator: function(model1, model2){
            return model1.get("name") < model2.get("name") ? -1 : 1;
        },
