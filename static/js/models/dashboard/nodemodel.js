...
define(["underscore", "backbone", "poller", "config/app-config", "helper/util"], function(_, Backbone, Poller, AppConfig, Util){
    var NodeModel = Backbone.Model.extend({
        ...
        url: function(){
            return AppConfig.baseUrl + window.AMCGLOBALS.persistent.clusterID + AppConfig.node.resourceUrl + this.address;
        },
        initialize :function(){
        ...
        fetchSuccess: function(response){
            ...
            try{
            	if(response.get('node_status') === 'on' && response.get('same_cluster') ==='False' ){
            		Util.showClusterDivertPopup("Node <strong>" + (response.get('address')) + "</strong> cannot be monitored here as it belongs to a different cluster");
            	}
