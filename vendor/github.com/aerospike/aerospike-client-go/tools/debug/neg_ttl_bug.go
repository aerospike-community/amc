...
func main() {
	...
	if client, err := NewClientWithPolicy(clientPolicy, *host, *port); err != nil {
		...
		// wPolicy := NewWritePolicy(0, 90*3600*24)
		wPolicy := NewWritePolicy(0, 2)
		key, _ := NewKey(*namespace, *set, 1)
		err := client.Put(wPolicy, key, BinMap{"Bin": 1})
		if err != nil {
			log.Fatal(err)
		}
		...
		fmt.Println("TTL is", rec.Expiration)
	}
	log.Println("-HAHA")
}
