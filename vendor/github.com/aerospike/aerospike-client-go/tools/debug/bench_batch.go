...
var batchKeys = flag.Int("keys", 50, "Number of keys in a batch request.")
...
func run() {
	...
	if *genData {
		...
		log.Println("Generating data finished successfully...", time.Since(start))
	}
	...
	policy.TotalTimeout = 150 * time.Millisecond

	// panic("Test with Timeouts")
	...
	for i := 0; i < cnt; i++ {
		go func() {
			...
			for ttt := 0; ttt < 1e4; ttt++ {
				b := rand.Intn(*keyRange - *batchKeys)
				keys := make([]*Key, *batchKeys)
				for k := b; k < b+*batchKeys; k++ {
					key, _ := NewKey(*namespace, *set, k)
					keys[k-b] = key
				}

				// tbeg := time.Now()
				results, err := client.BatchGet(policy, keys)
				// if t := time.Since(tbeg); t > policy.Timeout {
				// 	log.Println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~", (t - policy.Timeout).String())
				// }
				...
				for i, r := range results {
					if *verify {
						...
						if err := verifyResult(b+i, bins, r.Bins); err != nil {
							nerr++
							// log.Println(err.Error())
						}
					}
