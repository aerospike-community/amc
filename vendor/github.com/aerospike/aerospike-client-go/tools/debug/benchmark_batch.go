...
var batchConcNodes = flag.Int("BT", 0, "Batch Concorrenct Nodes. 0: Parallel 1: Sequnce N>1: N nodes in parallel")
var batchUseDirect = flag.Bool("BatchDirect", false, "Use Batch Direct.")
...
func main() {
	...
	clientPolicy.TendInterval = 10 * time.Second
	clientPolicy.LoginTimeout = 30 * time.Second
...
func runBench_RU(client *as.Client, ident int, times int) {
	...
	// batchPolicy.UseBatchDirect = *batchUseDirect
	batchPolicy.ConcurrentNodes = *batchConcNodes
	batchPolicy.Timeout = time.Duration(*timeout) * time.Millisecond
	// batchPolicy.SocketTimeout = 250 * time.Millisecond
	...
	for {
		...
		if int(xr.Uint64()%100) >= workloadPercent {
			...
			if !*useMarshalling {
				for j := range keys {
					key, _ := as.NewKey(*namespace, *set, as.IntegerValue(partition+((i+j)%times)))
					keys[j] = key
				}

				tm = time.Now()
				// _, err = client.Get(readpolicy, key, bin.Name)
				_, err = client.BatchGet(batchPolicy, keys)
				// if err != nil {
				// 	logger.Println(err)
				// }
			} else {
...
func reporter(client *as.Client) {
	statsCnt := 0
	...
	memProfileStr := func() string {
		...
		if *debugMode {
			...
			res = fmt.Sprintf(" (malloc (KiB): %d, GC pause(ms): %d, Gorountines: %d)",
				allocMem,
				pauseNs,
		...
		case stats := <-countReportChan:
			...
			if stats.Exit || time.Now().Sub(lastReportTime) >= time.Second {
				...
				if *latency != "" {
					...
					strBuff.WriteString(fmt.Sprintf("\tREAD\t%d\t%3.1f\t%d", rMinLat, float64(rTotalLat)/float64(totalRCount+1), rMaxLat))
					for i := 0; i <= latCols; i++ {
					...
					strBuff.WriteString(fmt.Sprintf("\tWRITE\t%d\t%3.1f\t%d", wMinLat, float64(wTotalLat)/float64(totalWCount+1), wMaxLat))
					for i := 0; i <= latCols; i++ {
