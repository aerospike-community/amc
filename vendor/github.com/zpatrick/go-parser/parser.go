			...
			for _, genSpec := range genDecl.Specs {
				...
				case *ast.TypeSpec:
					...
					case (*ast.StructType):
						...
						goStruct := buildGoStruct(source, goFile, info, typeSpec, structType, declType.Doc.Text())
...
func buildGoStruct(source []byte, file *GoFile, info *types.Info, typeSpec *ast.TypeSpec, structType *ast.StructType, docs string) *GoStruct {
	goStruct := &GoStruct{
		File:   file,
		Name:   typeSpec.Name.Name,
		Docs:   docs,
