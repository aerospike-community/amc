...
func NewTimeSeries(typ TSType, os ...Option) (*TimeSeries, error) {
	...
	return newTimeSeries(typ, opts.clock, opts.granularities)
}

func newTimeSeries(typ TSType, clock Clock, granularities []Granularity) (*TimeSeries, error) {
	err := checkGranularities(granularities)
	if err != nil {
		return nil, err
	}
	return &TimeSeries{typ: typ, clock: clock, levels: createLevels(typ, clock, granularities)}, nil
}
...
func createLevels(typ TSType, clock Clock, granularities []Granularity) []level {
	levels := make([]level, len(granularities))
	for i := range granularities {
		levels[i] = newLevel(typ, clock, granularities[i].Granularity, granularities[i].Count)
	}
	return levels
}
...
func (t *TimeSeries) increaseAtTime(amount float64, tm time.Time) {
	for i := range t.levels {
		if tm.Before(t.levels[i].latest().Add(-1 * t.levels[i].duration())) {
			continue
		}
		t.levels[i].increaseAtTime(amount, tm)
	}
}
